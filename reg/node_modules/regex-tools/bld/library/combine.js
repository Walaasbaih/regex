...
class CombinedResult {
    constructor(combined, groupNames, groupNameToIndex, groupNameHideMap) {
        this.combined = combined;
        this.groupNames = groupNames;
        this.groupNameToIndex = groupNameToIndex;
        this.groupNameHideMap = groupNameHideMap;
    }
    getRegexLiteral(flags) {
        let literal = `/${this.combined.replace(/\\.|(\/)/g, (m, g1) => (g1 ? '\\/' : m))}/`;
        return literal + (flags || '');
    }
    getParametersSnippet({ typed = false, matchName = 'text', separator = ', ', }) {
    ...
    getGroupAliasDeclarationsSnippet({ arrayName = 'groups', useLet = true, newLine = '\n', lineIndent = '', matchName = '', } = {}) {
        let lines = [];
        if (matchName) {
    ...
    getEnumDeclaration({ useConst = false, name = 'ExecGroup', newLine = '\n', lineIndent = '', indent = '    ', } = {}) {
        let lines = [];
        let hideMap = this.groupNameHideMap;
        let skipped = true;
...
function combine(regexes) {
    let groupCount = 0;
    let groupNameToIndex = {};
    let groupNameHideMap = {};
    let regexIndex = 0;
    ...
    for (let name of Object.keys(groupNameToIndex)) {
        groupNames[groupNameToIndex[name] - 1] = name.replace(/-([a-z])/gi, (_m, g1) => g1.toUpperCase());
    }
    return new CombinedResult(combined, groupNames, groupNameToIndex, groupNameHideMap);
    function processRegexes(regexes, upperOr) {
        let name;
        let regexArray;
        let or;
        let capture;
    ...
    function processPartialRegex(regex, upperOr) {
        ...
        let partialRegexStr = regexStr.replace(GROUP_REGEX, (text, groupName, groupNameColon, digitFollowsBR, braWithQ, bra, ket, or, sBra, sKet, brNumber) => {
            if (groupName) {
                if (sBraOpen) {
